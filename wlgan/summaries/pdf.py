...
def get_1pt_pdf(kappa_maps, kappa_bins):
    kappa_pdf = []
    nbins = kappa_maps.shape[0]
    for i in range(nbins):
        kappa_pdf_i = _singlebin_kappa_1pt_pdf(kappa_maps[i], kappa_bins[i])
        kappa_pdf.append(kappa_pdf_i)
...
def generate_kappa_bins(KAPPA_STD, N_KAPPA_BINS=61):
    ...
    nbins = len(KAPPA_STD)
    for i in range(nbins):
        kappa_bins_i = np.linspace(-3. * KAPPA_STD[i], 7. * KAPPA_STD[i], N_KAPPA_BINS)
        kappa_bins.append(kappa_bins_i)  
    return np.array(kappa_bins)

def save_summary_1pt_pdf(f, kappa_pdf, kappa_bins):
    summary_grp = f.create_group('1pt_pdf')
